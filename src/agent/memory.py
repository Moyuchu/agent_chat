...
class Memory:
    def __init__(self):
        ...
        self.rag_retriever = RAGRetriever(self.rag_index)

    async def add_memory(self, 
                        message: str, 
                        role: Literal["user", "assistant", "action_result"]) -> None:
        ...
        # rag
        self.rag_index.add_document(message)

        # 如果超出短期记忆大小，将旧记忆存入RAG
    ...
    def retrieve_relevant(self, query: str, top_k: int = 3) -> List[str]:
        return self.rag_retriever.retrieve(query, top_k)
